...
if isnumeric(boundaries)~=1
    errormsg=['Argument nine, ', boundaries,', should be numeric.'];
    disp(errormsg);
end
if size(offsetRGB,2)~=3
    if offsetRGB~=0
        disp(' ');
        errormsg=['Argument ten, ', num2str(offsetRGB), ', should have three entries.'];
        disp(errormsg);
...
elseif isnumeric(offsetRGB)~=1
    disp(' ');
    errormsg=['Argument ten, ', offsetRGB, ', should have three numeric entries.'];
    disp(errormsg);
    disp(['Setting offset color to ' num2str(doffsetRGB) '.']);
    offsetRGB=doffsetRGB;
...
if size(jumpRGB,2)~=3
    if jumpRGB~=0
        disp(' ');
        errormsg=['Argument eleven, ', num2str(jumpRGB), ', should have three entries.'];
        disp(errormsg);
...
elseif isnumeric(jumpRGB)~=1
    disp(' ');
    errormsg=['Argument eleven, ', jumpRGB, ', should have three numeric entries.'];
    disp(errormsg);
    disp(['Setting jump color to ' num2str(djumpRGB) '.']);
    jumpRGB=djumpRGB;
...
if size(trackRGB,2)~=3
    if trackRGB~=0
        disp(' ');
        errormsg=['Argument twelve, ', num2str(trackRGB), ', should have three entries.'];
        disp(errormsg);
...
elseif isnumeric(trackRGB)~=1
    disp(' ');
    errormsg=['Argument twelve, ', trackRGB, ', should have three numeric entries.'];
    disp(errormsg);
    disp(['Setting track color to ' num2str(dtrackRGB)'.']);
    trackRGB=dtrackRGB;
...
if size(bgRGB,2)~=3
    if bgRGB~=0
        disp(' ');
        errormsg=['Argument thirteen, ', num2str(bgRGB), ', should have three entries.'];
        disp(errormsg);
...
elseif isnumeric(bgRGB)~=1
    disp(' ');
    errormsg=['Argument thirteen, ', bgRGB, ', should have three numeric entries.'];
    disp(errormsg);
    disp(['Setting background color to ' num2str(dbgRGB) '.']);
    bgRGB=dbgRGB;
...
if scaling(1)<1
    if strcmp(graphtype, 'values')
        coefficient=scaling(1)*xlength/RoughnessAverage;
    elseif strcmp(graphtype, 'sqrt')
        coefficient=scaling(1)*xlength/sqrt(RoughnessAverage);
    elseif strcmp(graphtype, 'log')
        coefficient=scaling(1)*xlength/log2(RoughnessAverage);
    end
else

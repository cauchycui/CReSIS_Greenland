function PlotOffsetRoughnessRGB(varargin)
...
optinputs={0, 1, 0, 'values', 1, 'default', 'default', 0, [.8 0 0], [0 .8 .4], [1 1 1]};
...
if filename==0
    ...
    disp('PlotOffsetRoughness has up to 8 inputs: filename, start track, end track,')
    disp('linearization, symmetry, title, scaling, boundaries. If an argument is')
    disp('not entered, the default value is used.')
...
if mirror~=1
   if mirror~=2
        warning('Argument five should be a 1 or a 2.');
   end
end
...
if size(offsetRGB,2)~=3
    ...
    disp(errormsg);
    disp('Setting offset color to [1 0 0].');
    offsetRGB=[1 0 0];
...
if size(jumpRGB,2)~=3
    ...
    disp(errormsg);
    disp('Setting offset color to [0 1 0].');
    jumpRGB=[0 1 0];
...
if size(trackRGB,2)~=3
    ...
    disp(errormsg);
    disp('Setting offset color to [0 0 1].');
    trackRGB=[0 0 1];
...
for m=starttrack+1:endtrack+1
    ...
    plot(PlotMatrix1(:,1), PlotMatrix1(:,2),'Color', trackRGB);

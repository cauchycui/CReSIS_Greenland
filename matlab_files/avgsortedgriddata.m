function avgsortedgriddata(filename, varargin)
...
while(ischar(a_line))
    ...
    if isequal(current_box, datum(1:2))
        ...
        box_sum=box_sum+datum(3);
    else %if we have changed boxes, we'll print the data of the last box before storing new box data
        box_average=box_sum/data_count;
        fprintf(ofid, '%d %d %f %d ', current_box(1), current_box(2), box_average, data_count);

...
optinputs={0, 1, 0, 'values', 1, 'default', 'default', 0, [.8 0 0], [0 .8 .4], [.2 .6 1], [1 1 1]};
...
starttrack=optinputs{1,2};
endtrack=optinputs{1,3};
graphtype=optinputs{1,4};
mirror=optinputs{1,5};
usertitle=optinputs{1,6};
scaling=optinputs{1,7};
boundaries=optinputs{1,8};
offsetRGB=optinputs{1,9};
jumpRGB=optinputs{1,10};
trackRGB=optinputs{1,11};
bgRGB=optinputs{1, 12};
...
if filename==0
    ...
    disp('PlotOffsetRoughness has up to twelve inputs: filename, start track, end track,')
    disp('linearization, symmetry, title, scaling, boundaries, offset color, jump color, track')
    disp('color, and background color. If an argument is not entered, the default value is used.')
...
elseif strcmp(filename, 'help')
    ...
    disp('filename, start track, end track, linearization, title, scaling, and boundaries.')
...
elseif strcmp(filename, 'help start track') || strcmp(filename, 'help end track')
    disp('The start and end track values determine which and how many satellite tracks to')
    disp('plot. By default, the function plots all the tracks. The function will not work')
    disp('if an end track is given that is higher than the start track value or the')
    disp('number of tracks detected (displayed at the end of running the function).')
...
if fid==-1
    error('File not found, check argument one.')     %Exits the program if file is not found.
else
if mirror~=1
   if mirror~=2
...
if isnumeric(starttrack)~=1
    errormsg=['Argument two, ', starttrack, ', should be numeric.'];
    disp(' ');    disp(errormsg);
end
if isnumeric(endtrack)~=1
    errormsg=['Argument three, ', endtrack, ', should be numeric.'];
    disp(errormsg);
end
...
if size(offsetRGB,2)~=3
    if offsetRGB~=0
        ...
        disp(errormsg);
        disp('Setting offset color to [.8 0 0].');
    end
    offsetRGB=[.8 0 0];
...
elseif offsetRGB(1)>1 || offsetRGB(1)<0 || offsetRGB(2)>1 || offsetRGB(2)<0 || offsetRGB(3)>1 || offsetRGB(3)<0
    ...
    disp('Setting offset RGB to default');
end
...
if size(jumpRGB,2)~=3
    if jumpRGB~=0
        ...
        disp(errormsg);
        disp('Setting jump color to [0 .8 .4].');
    end
    jumpRGB=[0 .8 .4];
...
elseif jumpRGB(1)>1 || jumpRGB(1)<0 || jumpRGB(2)>1 || jumpRGB(2)<0 || jumpRGB(3)>1 || jumpRGB(3)<0
    ...
    disp('Setting jump RGB to default');
end
if size(trackRGB,2)~=3
    if trackRGB~=0
        ...
        disp(errormsg);
        disp('Setting track color to [.2 .6 1].');
    end
    trackRGB=[.2 .6 1];
...
elseif trackRGB(1)>1 || trackRGB(1)<0 || trackRGB(2)>1 || trackRGB(2)<0 || trackRGB(3)>1 || trackRGB(3)<0
    ...
    disp('Setting track RGB to default');
end
if size(bgRGB,2)~=3
    if bgRGB~=0
        disp(' ');
        ...
        disp(errormsg);
        disp('Setting background color to default.');
    end
    bgRGB=[1 1 1];
...
elseif bgRGB(1)>1 || bgRGB(1)<0 || bgRGB(2)>1 || bgRGB(2)<0 || bgRGB(3)>1 || bgRGB(3)<0
    ...
    disp('Setting background RGB to default');
end
...
%Here is where the scaling coefficient is determined
RoughnessAverage=mean(roughness);
xlength=max(PathMatrix(:,1))-min(PathMatrix(:,1));
...
if strcmp(graphtype,'values')    
    OffsetMatrix(:,1)=coefficient*roughness;
elseif strcmp(graphtype, 'sqrt')
    OffsetMatrix(:,1)=coefficient*sqrt(roughness);
elseif strcmp(graphtype, 'log')
    minimum=min(log2(roughness(BreakArray(starttrack):BreakArray(endtrack))));
    OffsetMatrix(:,1)=coefficient*(log2(roughness)-minimum);
end
for n=1:DataSize
...
for m=starttrack+1:endtrack+1
    ...
    %if symmetry is on
    PlotMatrix2(1:BreakArray(m)-BreakArray(m-1),1)=PathMatrix(BreakArray(m-1):BreakArray(m)-1,1)...
        + OffsetMatrix(BreakArray(m-1):BreakArray(m)-1,1);
    PlotMatrix2(1:BreakArray(m)-BreakArray(m-1),2)=PathMatrix(BreakArray(m-1):BreakArray(m)-1,2)...
        +OffsetMatrix(BreakArray(m-1):BreakArray(m)-1,2);
    if mirror>1
        MirrorMatrix=zeros((BreakArray(m)-BreakArray(m-1)), 2);
        MirrorMatrix(1:BreakArray(m)-BreakArray(m-1),1)=PathMatrix(BreakArray(m-1):BreakArray(m)-1,1)-OffsetMatrix(BreakArray(m-1):BreakArray(m)-1,1);
        MirrorMatrix(1:BreakArray(m)-BreakArray(m-1),2)=PathMatrix(BreakArray(m-1):BreakArray(m)-1,2)-OffsetMatrix(BreakArray(m-1):BreakArray(m)-1,2);
...
%to the starting and ending break points (starttrack and endtrack) and 
%labels them JumpStart and JumpEnd. JumpEnd has to find the break point
%after endtrack, so additional code is included in case endtrack is the
%final track
JumpEnd=0;
for n=1:Jumppoints
...
for m=starttrack+1:endtrack+1
    ...
    %for n=1:BreakArray(m)-BreakArray(m-1)
        PlotMatrix1(1:BreakArray(m)-BreakArray(m-1),1)=PathMatrix(BreakArray(m-1):BreakArray(m)-1,1);
        PlotMatrix1(1:BreakArray(m)-BreakArray(m-1),2)=PathMatrix(BreakArray(m-1):BreakArray(m)-1,2);
    %end
